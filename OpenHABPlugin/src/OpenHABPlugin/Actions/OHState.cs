namespace Loupedeck.OpenHABPlugin.Actions
{
    ...
    class OHState : PluginDynamicCommand
    {
        ...
        public OHState() : base("Get a state", "Get the state value of an item.", "")
        {
            this.MakeProfileAction("text;Enter item");

            // Reload the data periodically every 1 minutes
            this.timer = new Timer(10 * 1 * 1000);
        ...
        protected async void LoadData(String actionParameter)
        {
            ...
            try
            {
                ...
                var url = "http://" + OpenHABPlugin.Config.User + ":" + OpenHABPlugin.Config.Password + "@" + OpenHABPlugin.Config.Url + actionParameter;
